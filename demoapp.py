...
def visualize(top_row, color):
    print(
        f"""Top result for lexical search is elastic_id={top_row['arxiv_id']} with score={top_row['score']}\n"""
    )
...
def visualize_st(top_row, color):
    st.write(
        f"""Top result is **elastic_id={top_row['arxiv_id']}** with **score={top_row['score']}**\n"""
    )
    colored_text = colorize_st(top_row.abstract, query_keywords, color=color)
    st.write(colored_text)


st.title("Basic Search Tests")
st.markdown("## Input search query")
st.markdown(
    """
    Enter a query you'd like to use to search the simple elastic index and compare the various search results: 
    ...
    * Semantic search will use vectors to search the elastic cosine index  
    """
)
INDEX_NAME = "arxiv-cosine" 
query = st.text_input("**Query**", "tell me about PTSD?")
